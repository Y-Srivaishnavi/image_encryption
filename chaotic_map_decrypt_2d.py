...
while True: 
	...
	if choice_map == '1' or choice_map.casefold() in 'henon':
		choice_map = 'henon'
		a = 1.4
		b = 0.3
		break
		
	elif choice_map == '2' or choice_map.casefold() in 'standard':
		choice_map = 'standard'
		k = 2.7
		break
...
encrypted_img = cv2.imread("./encrypted_images/{}_encrypted_{}.png".format(choice_map, choice_image))
height, width, channels = plain_img.shape

n = height*width*channels

# Flatten image array
flat_array = encrypted_img.flatten()
...
for i in range(1, n):
	...
	if choice_map == 'henon':
		decrypted_array[i-1] = flat_array[i-1] ^ int(abs(y[i-1]) * 630)		# Range of henon is -0.38 to 0.38; 255 / 0.4 ~ 637
		
		x[i] = 1 + y[i-1] - a * (x[i-1] ** 2)
		y[i] = b * x[i-1] 
		
	elif choice_map == 'standard':
		decrypted_array[i-1] = flat_array[i-1] ^ int((abs(y[i-1]) % 1)*255 ) 
		
		x[i] = (x[i-1] + y[i-1]) % 1
		y[i] = y[i-1] + k * math.sin(2 * math.pi* x[i])
...
if choice_graphs.casefold() == 'y':
	...
	plot_corr_graphs(decrypted_image, plain_img)
